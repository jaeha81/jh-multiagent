...
로컬 연결(codex·gemini-cli MCP)은 `setup.sh` 참고 — 인증은 본인이 직접.
...
# Windows — PowerShell 7이 설치된 경우
pwsh -File _shared\run-selfcheck.ps1
```

```bash
# Linux / Mac
bash _shared/run-selfcheck.sh
```
...
마우스로 선택 가능한 자연어 개발 컨트롤 패널을 바로 열려면:

```powershell
powershell -ExecutionPolicy Bypass -File .\launch-web.ps1
```

웹 패널은 브라우저에서 `127.0.0.1` 로만 열리며, 좌측에서 폴더 진입, 링크 전송,
파일 업로드, 모델 선택, 기본 권한, 자동 검토, 골모드, 케이브맨 옵션을 선택할 수 있다.
중앙 입력창에 자연어 개발 요청을 넣으면 `tasks/` 아래에 실행 가능한 task 초안을 만든다.
worker 호출과 외부 repo 쓰기는 기존 승인 게이트를 유지한다.
