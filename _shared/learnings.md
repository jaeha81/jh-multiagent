...
## [2026-05-30] JH 배포 — gemini 브리지 마이그레이션

**교훈**: 원본 starter는 작성자 사설 `mcp__gemini-pro__*` 브리지를 전제했고 영상은 연결법을 비공개했다. JH 배포는 공개 브리지 `gemini-cli`(jamubc/gemini-mcp-tool)로 통일 — 도구는 `mcp__gemini-cli__ask-gemini` 하나, 텍스트·파일·이미지를 `@경로`로 처리(별도 vision 도구 없음), 기본 모델 `gemini-2.5-pro`. 브리지 교체 시 반드시 routing.md·task-folder.md·design-basis.md(D4)·system-invariants(INV9/INV10)를 함께 갱신하고 자가점검 통과 후 커밋. gemini는 `config.md` 토글로 끌 수 있으므로 브리지가 깨지면 즉시 off로 우회. 위 이전(2026-05-15/19/20) pro-high 관련 항목은 사설 브리지 한정 히스토리로, JH 환경엔 해당 없음.
