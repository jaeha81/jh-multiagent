...
- **호출 명령**: `mcp__codex__codex` MCP 도구
  - `prompt`: brief.md 내용 그대로 전달
  - `cwd`:
    - 기본: `~/JH-MultiAgent/tasks/<task>/` — 이 안에서 산출물·diff 직접 작성
...
- **결과물**: 분석 텍스트, 요약
- **호출 명령** (단일 브리지 `gemini-cli` = jamubc/gemini-mcp-tool. 텍스트·비전 공통):
  ```
  mcp__gemini-cli__ask-gemini    # prompt 필수. 파일·이미지는 prompt 안에서 @경로 로 참조
  mcp__gemini-cli__sandbox-test  # 코드 안전 실행(선택)
  # model: 기본 gemini-2.5-pro. 다른 모델 필요 시 model 파라미터 명시
  # 비전: 별도 도구 없음 — ask-gemini prompt 에 @이미지경로 포함
  # config.md gemini_enabled=true 일 때만. false 면 스킵 후 사용자에게 대체안 질의
  ```
- **비용**: 모델별 — gemini-2.5-pro 중간 / flash 낮음 → 승인 필요
...
- **gemini**: 단일 브리지 `gemini-cli`(jamubc/gemini-mcp-tool, Gemini CLI 래퍼)만 사용. 도구 `mcp__gemini-cli__ask-gemini` 하나로 텍스트·파일·이미지(`@경로`)를 모두 처리(별도 vision 도구 없음). 기본 모델 `gemini-2.5-pro`(브리지 기본값) — 다른 모델은 `model` 파라미터로. gemini 호출은 `config.md`의 `gemini_enabled`가 true일 때만(향후 토글 off 가능). 근거: `design-basis.md` D4·D6.
