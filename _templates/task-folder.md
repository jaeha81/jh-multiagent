...
- **gemini**: `mcp__gemini-cli__ask-gemini` 호출 (텍스트·`@경로` 파일/이미지 공통, model 기본 `gemini-2.5-pro`). `config.md` gemini_enabled=true 일 때만. Orchestrator가 응답을 `result.md`에 기록
- **codex-main / codex-critic**: `mcp__codex__codex` MCP 도구 호출
  - `prompt`: brief.md 내용 그대로
  - `cwd`:
    - codex-main 기본: `~/JH-MultiAgent/tasks/<task>/` (작업 폴더 안에서 산출물 작성)
